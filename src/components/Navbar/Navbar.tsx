    ...
    return (
        <>
            <nav className={classes.navbar}>
                <div
                    className={`${classes.navbar__container} container flex items-center justify-between `}
                >
                    <div className='flex items-center'>
                        <Link
                            href='/'
                            className={`text-xl  font-semibold text-emerald-100 hover:text-orange-600 `}
                        >
                            {LOGO}
                        </Link>
                    </div>
...
const NavLink = ({ href, title }) => {
    return (
        <Link
            href={href}
            className={`text-white hover:text-orange-700 ${classes.navbar__link}`}
        >
            {title}
        </Link>
    );
